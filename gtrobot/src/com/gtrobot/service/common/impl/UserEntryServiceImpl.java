...
public class UserEntryServiceImpl implements UserEntryService {
    ...
    private final SortedSet<String> activeUserList = new TreeSet<String>();

    private UserEntryManager userEntryManager;

    public void setUserEntryManager(final UserEntryManager userEntryManager) {
    ...
    public UserEntry getUserEntry(String jid) {
        ...
        UserEntry userEntry = this.userEntryManager.getUserEntry(jid);
        if (userEntry == null) {
            final GoogleTalkConnection googleTalkConnection = GTRobotContextHelper
                    .getGoogleTalkConnection();
            final Roster roster = googleTalkConnection.getXMPPConnection()
                    .getRoster();
            final RosterEntry rosterEntry = roster.getEntry(jid);
            if (rosterEntry != null) {
                userEntry = new UserEntry();
                userEntry.setJid(jid);
                userEntry.setNickName(rosterEntry.getName());

                this.userEntryManager.saveUserEntry(userEntry);
            }
        }
        return userEntry;
    }

    public synchronized boolean updateNickname(final UserEntry userEntry,
            final String newNickname) {
        final UserEntry userEntryTemp = this.userEntryManager
                .getUserEntryByNickName(newNickname);
    ...
    public void updateUserEntryPresence(final UserEntry userEntry,
        ...
        if (Presence.Type.available.equals(presence.getType())
            ...
            this.activeUserList.add(userEntry.getJid());
        } else {
            this.activeUserList.remove(userEntry.getJid());
        }
    }
